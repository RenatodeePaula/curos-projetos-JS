...
for(let i = 0; i < columns.length; i++) {
    ...
    column.addEventListener('drop', function(event) {
        ...
        const preventColumn = draggedTask.parentNode
        preventColumn.removeChild(draggedTask)
    })

}

const addTaskForm = document.querySelector('#add-task-form')
const addTaskInp = document.querySelector('input')

addTaskForm.addEventListener('submit', (e) => {
    e.preventDefault()
    const newTaskText = addTaskInp.value.trim()
